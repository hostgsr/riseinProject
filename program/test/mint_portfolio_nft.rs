...
mod mint_portfolio_nft {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("portfolio_nft", program_id)
			.start();
		...
		let seller_fee_basis_points: u64 = Default::default();
		let image: String = Default::default();
		let collection: String = Default::default();
		let properties: String = Default::default();
		let profession: String = Default::default();
		let skills: String = Default::default();
		let json_info: String = Default::default();
		...
		let (portfolio_pubkey, _) = Pubkey::find_program_address(
			&[b"portfolio", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
		...
		let data = PortfolioNftInstruction::MintPortfolioNft(crate::generated::instructions::MintPortfolioNftArgs{
			name,
			symbol,
			description,
